...
# URL to retrieve attachments - optional.  If not provided, Foswiki pub path is used.
    ...
    $perlCmd =
         $Foswiki::cfg{DirectedGraphPlugin}{perlCmd}
      || $Foswiki::cfg{Plugins}{DirectedGraphPlugin}{perlCmd}
      || 'perl';
...
#  This code overrides the target in the generated svg file for consistent operation.
    ...
    if ( ($attachPath) && ($attachUrlPath) && !( $forceAttachAPI eq "on" ) ) {
        ...
        $urlPath = Foswiki::Func::getPubUrlPath();
    }
...
"new  $filename ... types= $newHash{FORMATS}{$filename} "
            ...
            Foswiki::Func::clearSessionValue('DGP_hash');
            Foswiki::Func::clearSessionValue('DGP_newhash');
        }    ### if ($newHash{SET}
    }    ### if ($newHashRef)
...
#   the Web name and Topic name.  On older versions of Foswiki, it simply deleted the files
...
#
# _attachmentExists
#    Check if attachment exists - use Foswiki API or direct file I/O
#
sub _attachmentExists {
    ...
    if ( ($attachPath) && !( $forceAttachAPI eq "on" ) ) {
        ...
        return Foswiki::Func::attachmentExists( $web, $topic, $fn );
    }
}
